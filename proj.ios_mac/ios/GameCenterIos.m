...
{
    UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Game Center" message:error.localizedDescription delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alert show];
    
    NSUserDefaults* preferences = [NSUserDefaults standardUserDefaults];
    NSString* isLoggedInKey = @"isLoggedIn";
    const NSInteger isLoggedIn = 0;
    [preferences setInteger:isLoggedIn forKey:isLoggedInKey];
    ...
    [localPlayer authenticateWithCompletionHandler:^(NSError* error) {
        if (error) {
            if (error.code != GKErrorAuthenticationInProgress) {
                ...
                [self showLoginFailedAlert:error];
            }
        } else {
